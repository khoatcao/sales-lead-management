...
from app.models.models import LeadStatusEnum, PriorityEnum, User
...
async def list_leads(
    ...
    status: LeadStatusEnum | None = Query(None),
    priority: PriorityEnum | None = Query(None),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    leads, total = await LeadService(db).list(page, per_page, current_user, status, priority)
